		...
		for (unsigned b = 0; b < i_neural_network[a].size(); b++)
		{
			...
			else if (b >= BIAS_NEURONS[a])
			{
				...
				for (unsigned c = next_bias_neurons; c < i_neural_network[1 + a].size(); c++)
				{
					...
					for (unsigned k = 0; k < i_weights[a][b - next_bias_neurons].size(); k++)
					{
						neuron_output += i_neural_network[a][k] * i_weights[a][b - next_bias_neurons][k];
					}
					i_errors[a - 1][b - bias_neurons] += i_errors[a][c - next_bias_neurons] * i_weights[a][c - next_bias_neurons][b - bias_neurons] * activation_function(1, neuron_output);
				}
			}
